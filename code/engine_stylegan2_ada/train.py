...
class Config:
    ...
    workers: int = 0
    xflip: bool = True
    paired: bool = False           # True -> 2-channel (image+mask) PairedSliceDataset for TSTR
    ...
    resume: str = None             # path to a latest.pt (full state) to resume from; None -> fresh run
...
def train(cfg: Config):
    ...
    # --- data ---
    if cfg.paired:
        from .data.paired_dataset import PairedSliceDataset
        ds = PairedSliceDataset(cfg.data, resolution=cfg.resolution, xflip=cfg.xflip)
    else:
        ds = SliceDataset(cfg.data, resolution=cfg.resolution, xflip=cfg.xflip)
    print(f"Dataset: {len(ds)} images (xflip={cfg.xflip}) @ {cfg.resolution}px")
    ...
    # --- resume from a full-state checkpoint (crash recovery) ---
    if cfg.resume and os.path.exists(cfg.resume):
        ...
        if ck.get("scaler") is not None: scaler.load_state_dict(ck["scaler"])
        cur_img = int(ck["cur_img"]); step = int(ck["step"])
        aug.p = float(ck["aug_p"]); ada_stat = float(ck.get("ada_stat", 0.0))
        print(f"RESUMED from {cfg.resume} at kimg {cur_img/1000:.1f}/{cfg.kimg} (p={aug.p:.3f})", flush=True)
    ...
    while cur_img < total_imgs:
        ...
        if (cur_img // 1000) > 0 and (cur_img % (cfg.ckpt_every_kimg * 1000)) < cfg.batch:
            ...
            _save_full(out_dir / "checkpoints" / "latest.pt", G, D, G_ema, g_opt, d_opt,
                       scaler, cur_img, step, aug.p, ada_stat, cfg)
...
def _save_ckpt(G_ema, cfg, path):
    torch.save({"G_ema": G_ema.state_dict(), "config": asdict(cfg),
                "img_channels": int(G_ema.img_channels)}, path)


def _save_full(path, G, D, G_ema, g_opt, d_opt, scaler, cur_img, step, aug_p, ada_stat, cfg):
    """Full training state for crash-resume (atomic write: tmp -> replace)."""
    tmp = str(path) + ".tmp"
    torch.save({"G": G.state_dict(), "D": D.state_dict(), "G_ema": G_ema.state_dict(),
                "g_opt": g_opt.state_dict(), "d_opt": d_opt.state_dict(),
                "scaler": scaler.state_dict() if scaler is not None else None,
                "cur_img": int(cur_img), "step": int(step), "aug_p": float(aug_p),
                "ada_stat": float(ada_stat), "config": asdict(cfg),
                "img_channels": int(G_ema.img_channels)}, tmp)
...
def _save_samples(G_ema, grid_z, out_dir, cur_img, run):
    ...
    # paired multi-channel (image modalities + mask): tile every channel as its own grayscale row
    if imgs.shape[1] not in (1, 3):
        imgs = torch.cat([imgs[:, c:c + 1] for c in range(imgs.shape[1])], dim=0)
